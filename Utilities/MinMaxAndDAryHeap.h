...
template<class It>
using iterator_value_type = std::remove_reference_t<decltype(*std::declval<It>())>;
template<class T>
...
template<typename It, typename Compare>
void push_down_min(It begin, heap_detail::iterator_value_type<It> value, uint64_t index, uint64_t length, Compare && compare)
{
    using std::swap;
    for (;;)
    {
        ...
        if (last_grandchild < length)
        {
            ...
            if (!compare(begin[smallest], value))
                break;
            begin[index] = heap_detail::move(begin[smallest]);
            index = smallest;
        ...
        else
        {
            ...
            if (compare(begin[parent], value))
            {
                begin[index] = heap_detail::move(begin[parent]);
                index = parent;
            }
            break;
        }
    }
    begin[index] = heap_detail::move(value);
}
...
template<typename It, typename Compare>
void push_down_max(It begin, heap_detail::iterator_value_type<It> value, uint64_t index, uint64_t length, Compare&& compare)
{
    using std::swap;
    for (;;)
    {
        ...
        if (last_grandchild < length)
        {
            ...
            if (!compare(value, begin[largest]))
                break;
            begin[index] = heap_detail::move(begin[largest]);
            index = largest;
            uint64_t parent = parent_index(index);
...
void push_minmax_heap(It begin, It end, Compare&& compare)
{
    ...
    heap_detail::iterator_value_type<It> value = heap_detail::move(end[-1]);
            ...
            {
                {
                    ...
                    {
                        begin[index] = heap_detail::move(begin[grandparent]);
                        index = grandparent;
                    }
                    else
                        break;
...
void pop_minmax_heap_min(It begin, It end, Compare&& compare)
{
    ...
    if (length == 0)
        return;
    minmax_heap_helpers::push_down_min(begin, heap_detail::exchange(end[-1], heap_detail::move(begin[0])), 0, length, compare);
}
...
void pop_minmax_heap_max(It begin, It end, Compare&& compare)
{
    ...
    minmax_heap_helpers::push_down_max(begin, heap_detail::exchange(end[-1], heap_detail::move(begin[index])), index, length, static_cast<Compare&&>(compare));
}
...
void make_minmax_heap(It begin, It end, Compare && compare)
{
    ...
    if (length != 4)
    {
        ...
        for (;;)
        {
            ...
            uint64_t loop_until = heap_detail::max(lowest_index_with_no_grandchildren,
    ...
    switch (highest_bit & 1)
    {
        for (;;)
        {
            case 0:
                ...
                {
                    --index;
                    minmax_heap_helpers::push_down_min(begin, heap_detail::move(begin[index]), index, length, compare);
                    if (index == loop_until)
                        break;
                }
            if (index == 0)
            ...
            case 1:
                ...
                {
                    --index;
                    minmax_heap_helpers::push_down_max(begin, heap_detail::move(begin[index]), index, length, compare);
                    if (index == loop_until)
                        break;
                }
            loop_until /= 2;
...
void make_dary_heap(It begin, It end, Compare && compare)
{
    ...
    for (;; --index)
    {
        heap_detail::iterator_value_type<It> value = heap_detail::move(begin[index]);
        ...
        for (;;)
        {
            ...
            if (!compare(value, *largest_child))
                break;
            begin[move_down_index] = heap_detail::move(*largest_child);
            move_down_index = largest_child - begin;
        }
        begin[move_down_index] = heap_detail::move(value);
        if (index == 0)
            break;
    }
}
...
void pop_dary_heap(It begin, It end, Compare && compare)
{
    ...
    heap_detail::iterator_value_type<It> value = heap_detail::move(end[-1]);
    end[-1] = heap_detail::move(begin[0]);
    uint64_t index = 0;
    for (;;)
    {
        ...
        if (last_child < length)
        {
            ...
            if (!compare(value, *largest_child))
                break;
            begin[index] = heap_detail::move(*largest_child);
