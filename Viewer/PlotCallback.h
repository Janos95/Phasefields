...
struct PlotCallback {
    ...
    Solver::Status::Value operator()(Solver::IterationSummary const&) {
        for(size_t j = 0; j < problem.objectives.size(); ++j) {
            Functional& f = problem.objectives[j];
            double cost = 0;
            f(node.phasefield(), node.temporary(), cost, nullptr, nullptr);
            arrayAppend(costs[j], cost);
        }
    ...
    void generatePlot() {
        ...
        for(size_t k = 0; k < costs.size(); ++k) {
            ...
            plot += "};\n";
            plot += Cr::Utility::formatString("\\addlegendentry{{ {} }}", FunctionalType::to_string(problem.objectives[k].functionalType));
        }
        plot += "\n\\end{axis}\n"
                "\\end{tikzpicture}";
