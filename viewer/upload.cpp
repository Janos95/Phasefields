...
namespace detail {
    void compileInternal(Trade::MeshData&& meshData, Object& object, const CompileFlags flags) {
        object.vertices.setData(meshData.vertexData());
        object.indices.setData(meshData.indexData());
...
#define PHASEFIELDS_EXPORT __attribute__ ((visibility("default")))
//#define PHASEFIELDS_EXPORT

PHASEFIELDS_EXPORT Object upload(Trade::MeshData&& meshData, CompileFlag flags) {
    Object object;
    ...
    if(meshData.primitive() == MeshPrimitive::Triangles && (flags & (CompileFlag::GenerateFlatNormals|CompileFlag::GenerateSmoothNormals))) {
        CORRADE_ASSERT(meshData.attributeCount(Trade::MeshAttribute::Position),
                       "MeshTools::compile(): the mesh has no positions, can't generate normals", Object());
        ...
        CORRADE_ASSERT(meshData.attributeFormat(Trade::MeshAttribute::Position) == VertexFormat::Vector3,
                       "MeshTools::compile(): can't generate normals for"
                               << meshData.attributeFormat(Trade::MeshAttribute::Position) << "positions", Object());
        ...
        detail::compileInternal(std::move(generated), object, CompileFlag::GenerateFlatNormals);
    } else
        detail::compileInternal(std::move(meshData), object, CompileFlag::GenerateFlatNormals);
