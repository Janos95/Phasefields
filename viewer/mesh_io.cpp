...
void MeshIO::drawImGui(Viewer&) {
    if(ImGui::TreeNode("Mesh IO")){
        ...
        ImGui::InputText("Mesh Path (Saving)", m_outputMesh.data(), m_outputMesh.size());
        if(ImGui::Button("load mesh")){
            loadMesh(std::string{m_inputMesh.begin(), std::find(m_inputMesh.begin(), m_inputMesh.end(), '\0')});
        }
        ImGui::TreePop();
    }
}
