    ...
    public:
        inline static void PushBack(FilenameStringVector &rv, const char *s) {
            if (rv.Size == rv.Capacity)
                rv.reserve(rv._grow_capacity(rv.Size + 1));    // optional optimization from ImVector<>::push_back()
...
*/
    ...
    const SortingHelper::SorterSignature SortingHelper::Sorters[] = {&SortingHelper::Alphasort,
    ...
    class Directory {
    public:
        static void GetDirectories(const char *directoryName, PathStringVector &result,
...
#       endif //_WIN32
            ...
            if (n >= 0) {
                ...
                for (int cnt = 0; cnt < n; ++cnt) {
                    ...
                    sz = strlen(pName);
                    if (sz > 0) {
                        if (strcmp(pName, ".") != 0 && strcmp(pName, "..") != 0 && pName[0] != '.' &&
                            pName[sz - 1] != '~'
...
#               endif //__EMSCRIPTEN__
        ...
        static void
        GetFiles(const char *directoryName, PathStringVector &result, FilenameStringVector *pOptionalNamesOut = NULL,
                 Sorting sorting = SORT_ORDER_ALPHABETIC) {
            result.clear();
            if (pOptionalNamesOut) pOptionalNamesOut->clear();
            static char tempString[MAX_PATH_BYTES];
...
#       endif //_WIN32
        ...
        static void GetFiles(const char *path, PathStringVector &files, const char *wantedExtensions,
                             const char *unwantedExtensions = NULL, FilenameStringVector *pOptionalNamesOut = NULL,
                             Sorting sorting = SORT_ORDER_ALPHABETIC) {
            PathStringVector filesIn;
            FilenameStringVector namesIn;
            GetFiles(path, filesIn, &namesIn, sorting);
            if ((wantedExtensions == 0 || strlen(wantedExtensions) == 0) &&
...
#       endif //_WIN32
        ...
        inline static const PathStringVector &
        GetUserKnownDirectories(const FilenameStringVector **pOptionalUserKnownDirectoryDisplayNamesOut,
            ...
            for (int mntLocIndex = 0, sz = 2 * mountLocationSize; mntLocIndex < sz; mntLocIndex++) {
                ...
                for (int i = 0, msz = mediaFolders.size(); i < msz; i++) {
                    if (strcmp(mediaFolders[i], temp) == 0)
                        continue;  // I we have processed "/media/myusername" once cycle before, exclude it from processing "/media" subfolders
...
void PathGetAbsoluteWithZipSupport(const char* path,char* rv)   {
    ...
    bool
    FileGetContentBase(const char *path, ImVector<CharType> &bufferOut, bool openInTextMode, const char *password) {
        bufferOut.clear();
...
#   endif //IMGUI_USE_MINIZIP
        ...
        if ((sizeof(long) > sizeof(size_t) && szl != (long) sz) ||
    ...
    bool FileGetContent(const char *path, ImVector<unsigned char> &bufferOut, bool openInTextMode,
                        const char *password) {
...
// Internal Usage----------------------------------------------------------------------------------------
    struct ImGuiFsDrawIconStruct {
        ...
        inline void
        fillExtensionTypesFromFilenames(ImVector<int> &fileExtensionTypes, const FilenameStringVector &fileNames) {
        ...
        inline bool drawIcon(int extensionType, const ImVec4 *pOptionalColorOverride = NULL) const {
            return ImGuiFs::Dialog::DrawFileIconCallback ? ImGuiFs::Dialog::DrawFileIconCallback(extensionType,
                                                                                                 pOptionalColorOverride)
                                                         : false;
        }

        inline bool
        drawIcon(const char *ext, bool caseSensitiveMatch = false, const ImVec4 *pOptionalColorOverride = NULL) const {
    ...
    void
    FileGetExtensionTypesFromFilenames(ImVector<int> &fileExtensionTypesOut, const FilenameStringVector &fileNames) {
...
// End Internal Usage-------------------------------------------------------------------------------------
    ...
    Dialog::Dialog(bool noKnownDirectoriesSection, bool noCreateDirectorySection, bool noFilteringSection,
                   bool detectKnownDirectoriesAtEachOpening, bool addDisplayByOption,
...
// 90% of the functionality of the whole imguifilesystem.cpp is inside this single method
    const char *ChooseFileMainMethod(Dialog &ist, const char *directory, const bool _isFolderChooserDialog,
                                     const bool _isSaveFileDialog, const char *_saveFileName,
                                     const char *fileFilterExtensionString, const char *windowTitle,
                                     const ImVec2 &windowSize, const ImVec2 &windowPos, const float windowAlpha) {
        //-----------------------------------------------------------------------------
        ...
        bool &allowDirectoryCreation = I.allowDirectoryCreation = I.forbidDirectoryCreation ? false : (
                isSelectFolderDialog || isSaveFileDialog);
        //----------------------------------------------------------
        ...
        // Fill ColorSet above and fix dummyButtonColor here
        {
            ...
            for (int i = (int) Internal::ImGuiCol_Dialog_File_Background, sz = (int) Internal::ImGuiCol_Dialog_File_Text;
                 i <= sz; i++) {
                ImVec4 &c = ColorSet[i];
                const ImVec4 &r = style.Colors[i < sz ? ((int) ImGuiCol_Button -
                                                         (int) Internal::ImGuiCol_Dialog_File_Background + i)
                                                      : ImGuiCol_Text];
                Internal::ColorCombine(c, r, ff);
                if (i < sz) c.w *= ff.w;
            }
            for (int i = (int) Internal::ImGuiCol_Dialog_ZipDirectory_Background, sz = (int) Internal::ImGuiCol_Dialog_ZipDirectory_Text;
                 i <= sz; i++) {
                ImVec4 &c = ColorSet[i];
                const ImVec4 &r = style.Colors[i < sz ? ((int) ImGuiCol_Button + i) : ImGuiCol_Text];
...
#       endif //IMGUI_USE_MINIZIP
            ...
            if (I.detectKnownDirectoriesAtEveryOpening)
                pUserKnownDirectories = &Directory::GetUserKnownDirectories(&pUserKnownDirectoryDisplayNames,
                                                                            &pNumberKnownUserDirectoriesExceptDrives,
                                                                            true);
        ...
        if (I.forceRescan) {
            I.forceRescan = false;
#       ifndef IMGUI_USE_MINIZIP
            const int sortingModeForDirectories = (I.sortingMode <= (int) SORT_ORDER_LAST_MODIFICATION_INVERSE)
                                                  ? I.sortingMode : (I.sortingMode % 2);
            Directory::GetDirectories(I.currentFolder, I.dirs, &I.dirNames,
                                      (Sorting) sortingModeForDirectories);  // this is because directories don't return their size or their file extensions (so if needed we sort them alphabetically)
            ...
            if (!isSelectFolderDialog) {
                if (!fileFilterExtensionString || strlen(fileFilterExtensionString) == 0)
                    ...
                    Directory::GetFiles(I.currentFolder, I.files, fileFilterExtensionString, NULL, &I.fileNames,
                                        (Sorting) I.sortingMode);
                if (Dialog::DrawFileIconCallback)
                    ImGuiFsDrawIconStruct::Get().fillExtensionTypesFromFilenames(I.fileExtensionTypes, I.fileNames);
            } else {
                I.files.clear();
                I.fileNames.clear();
                I.fileExtensionTypes.clear();
...
#       endif //DEBUG_HISTORY
        ...
        const bool isBrowsingInsideZipFile = (fi && (!(fi->splitPathIndexOfZipFile < 0 ||
        ...
        bool editLocationInputTextReturnPressed = false;
        {
            ...
            if (I.editLocationCheckButtonPressed) {
                ImGui::SameLine();
                editLocationInputTextReturnPressed = ImGui::InputText("##EditLocationInputText",
                                                                      I.editLocationInputText, MAX_PATH_BYTES,
        ...
        if (I.allowKnownDirectoriesSection && pUserKnownDirectories->size() > 0) {
            ...
            if (ImGui::CollapsingHeader("Known Directories##imguifs_UserKnownDirectories")) {
                ...
                for (int i = 0, sz = (int) pUserKnownDirectories->size(); i < sz; i++) {
                    ...
                    if (ImGui::SmallButton(userKnownFolderDisplayName) &&
...
#           endif //SIMULATING_ONLY
        ...
        //-----------------------------------------------------------------------------
        {
            if (ImGui::BeginChild("BrowsingFrame", ImVec2(0, (isSaveFileDialog || isSelectFolderDialog) ? (
                ...
                I.calculateBrowsingDataTableSizes(
                        ImGui::GetWindowSize());  // (Actually we could save this call every frame, but it's difficult to detect whn we need it)
                ...
                if (I.dirs.size() > 0) {
                    ...
                    for (int i = 0, sz = (int) I.dirs.size(); i < sz; i++) {
                        ...
                        if (I.filter.PassFilter(dirName)) {
                            if (Dialog::DrawFolderIconCallback &&Dialog::DrawFolderIconCallback(false,
                                                                                                &iconsColor))
                                ImGui::SameLine();
...
#                   endif //IMGUI_USE_MINIZIP
                            if (Dialog::DrawFileIconCallback &&Dialog::DrawFileIconCallback(I.fileExtensionTypes[i],
                                                                                            pIconsColor))
                                ImGui::SameLine();
...
#                   endif //(defined(__EMSCRIPTEN__) && defined(EMSCRIPTEN_SAVE_SHELL))
        ...
        if (isSaveFileDialog || isSelectFolderDialog) {
            ...
            if (isSaveFileDialog) {
                ...
                ImGui::SameLine();
                lastTwoButtonsWidth =
                        ImGui::CalcTextSize("Save Cancel").x + 2.0f * (style.FramePadding.x + style.ItemSpacing.x) +
                ...
                if (ImGui::IsItemHovered() && I.mustFilterSaveFilePathWithFileFilterExtensionString &&
                ...
                ImGui::SameLine();
                lastTwoButtonsWidth =
                        ImGui::CalcTextSize("Select Cancel").x + 2.0f * (style.FramePadding.x + style.ItemSpacing.x) +
                        style.WindowPadding.x;
                ImGui::PushItemWidth(ImGui::GetContentRegionAvail().x - lastTwoButtonsWidth);
                ImGui::InputText("##selectFileNameDialog", &I.saveFileName[0], MAX_FILENAME_BYTES,
            ...
            if (ImGui::Button("Cancel")) { mustCancel = true; }
            ...
            if (selectionButtonPressed) {
                if (isSelectFolderDialog) {
                    ...
                    if (strlen(I.saveFileName) > 0) {
                        bool pathOk = true;
                        if (I.mustFilterSaveFilePathWithFileFilterExtensionString && fileFilterExtensionString &&
...
#       endif // IMGUI_USE_MINIZIP
    ...
    const char *
    Dialog::chooseFileDialog(bool dialogTriggerButton, const char *directory, const char *fileFilterExtensionString,
                             const char *windowTitle, const ImVec2 &windowSize, const ImVec2 &windowPos,
        ...
        if (dialogTriggerButton || (!internal->rescan && internal->open && strlen(getChosenPath()) == 0)) {
            const char *cp = ChooseFileMainMethod(*this, directory, false, false, "", fileFilterExtensionString,
...
#           endif // IMGUI_USE_MINIZIP
    ...
    const char *Dialog::chooseFolderDialog(bool dialogTriggerButton, const char *directory, const char *windowTitle,
                                           const ImVec2 &windowSize, const ImVec2 &windowPos, const float windowAlpha) {
        if (dialogTriggerButton) {
            internal->open = internal->rescan = internal->forceSetWindowPositionAndSize = true;
            internal->chosenPath[0] = '\0';
        ...
        if (dialogTriggerButton || (!internal->rescan && internal->open && strlen(getChosenPath()) == 0)) {
            const char *cp = ChooseFileMainMethod(*this, directory, true, false, "", "", windowTitle, windowSize,
                                                  windowPos, windowAlpha);
            if (!internal->open) {
                internal->userHasJustCancelledDialog = true;
#           ifdef IMGUI_USE_MINIZIP
                internal->unz.close();
#           endif // IMGUI_USE_MINIZIP
    ...
    const char *
    Dialog::saveFileDialog(bool dialogTriggerButton, const char *directory, const char *startingFileNameEntry,
                           const char *fileFilterExtensionString, const char *windowTitle, const ImVec2 &windowSize,
                           const ImVec2 &windowPos, const float windowAlpha) {
        if (dialogTriggerButton) {
            internal->open = internal->rescan = internal->forceSetWindowPositionAndSize = true;
            internal->chosenPath[0] = '\0';
            internal->userHasJustCancelledDialog = false;
        ...
        if (dialogTriggerButton || (!internal->rescan && internal->open && strlen(getChosenPath()) == 0)) {
            const char *cp = ChooseFileMainMethod(*this, directory, false, true, startingFileNameEntry,
                                                  fileFilterExtensionString, windowTitle, windowSize, windowPos,
                                                  windowAlpha);
            if (!internal->open) {
                internal->userHasJustCancelledDialog = true;
#           ifdef IMGUI_USE_MINIZIP
...
// just expose some methods
    ...
    void DirectoryGetDirectories(const char *directoryName, PathStringVector &result,
                                 FilenameStringVector *pOptionalNamesOut, Sorting sorting) {
        Directory::GetDirectories(directoryName, result, pOptionalNamesOut, sorting);
    }

    void DirectoryGetFiles(const char *directoryName, PathStringVector &result, FilenameStringVector *pOptionalNamesOut,
                           Sorting sorting) { Directory::GetFiles(directoryName, result, pOptionalNamesOut, sorting); }
