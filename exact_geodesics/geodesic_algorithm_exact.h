...
public:
    ...
    void initialize(std::vector<SurfacePoint>& sources, double max_propagation_distance);
    Status step(std::array<int, 2>& nodes, std::array<double,2>& dists);
...
private:
    ...
    IntervalWithStop m_candidates[2];
};
...
inline void GeodesicAlgorithmExact::initialize(std::vector<SurfacePoint>& sources, double max_propagation_distance){
    set_stop_conditions(nullptr, max_propagation_distance);
    set_sources(sources);
...
inline Status GeodesicAlgorithmExact::step(std::array<int, 2>& nodes, std::array<double,2>& dists){
    if(m_queue.empty()) return Status::Done;
    ...
    auto start = min_interval->start();
    auto stop = min_interval->stop();
    auto length = edge->length();
    Status status = Status::Continue;
    if(start < std::numeric_limits<double>::epsilon()) {
        nodes[0] = edge->v0()->id();
        dists[0] = min_interval->signal(0.0);
        status = Status::NewNode;
    }
    if(length - stop < std::numeric_limits<double>::epsilon()){
        nodes[1] = edge->v1()->id();
        dists[1] = min_interval->signal(length);
        status = Status::NewNode;
    }

    if(min_interval->min() >= m_max_propagation_distance)
        return Status::Done;
    ...
    for(unsigned i=0; i<edge->adjacent_faces().size(); ++i)		//two possible faces to propagate
    {
        ...
        unsigned num_propagated = compute_propagated_parameters(min_interval->pseudo_x(),
                                                                ...
                                                                turn_left,
                                                                turn_right,
                                                                m_candidates);        //if it is the last interval on the edge
        bool propagate_to_right = true;

        if (num_propagated) {
            if (m_candidates[num_propagated - 1].stop() != next_edge->length()) {
                propagate_to_right = false;
            }

            bool const invert =
                    next_edge->v0()->id() != edge->v0()->id(); //if the origins coinside, do not invert intervals
        ...
        if (propagate_to_right) {
            ...
            num_propagated = compute_propagated_parameters(length - min_interval->pseudo_x(),
                                                           ...
                                                           length -
                                                           min_interval->start(),        //start/end of the interval
																	...
																	interval_pointer source_interval)	//up to two candidates
{
    ...
    //if(candidates->stop() < std::numeric_limits<double>::epsilon())
	...
	else				//now we have to invert the intervals 
	{
		for(unsigned i=0; i<num_candidates; ++i)
		{
			...
			//if(p->stop() < std::numeric_limits<double>::epsilon())
			//    std::cout << "hello" << std::endl;
			...
			assert(p->start() <= p->stop()); //@todo there is something very fishy here
			assert(p->start() >= 0.0);
