...
template<int Width>
struct BucketInfo {
    static_assert(Width < 7);
    static constexpr hashMask = 1 << Width - 1;
    static constexpr emptyMask = 1 << Width;
    static constexpr distanceMask = ~(1 << (Width + 1) - 1);

    bool compareHash(BucketInfo const& other) const {
        return data & hashMask == other.data & hashMask;
    }

    bool compareDistance(BucketInfo const& other) const {
...
struct HashMap {
    ...
    static constexpr bool StoresTruncatedHash = BucketType::StoresTruncatedHash;
    ...
    Pair<iterator, bool> tryEmplace(Corrade::Containers::InPlaceInitT, Key&& key, Args&& ... args) {
        ...
        if(buckets[idx].empty()){
            ...
            buckets[idx].setEmptyBucket(distFromIdealBucket, hash, detail::move(value));
        }
