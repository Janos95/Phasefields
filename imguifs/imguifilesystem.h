...
const int MAX_FILENAME_BYTES =
        FILENAME_MAX*4;  // Worst case: 4 bytes per char, but huge waste of memory [we SHOULD have used imguistring.h!]
...
public:
    ...
    IMGUI_API Dialog(bool noKnownDirectoriesSection = false, bool noCreateDirectorySection = false,
                     bool noFilteringSection = false, bool detectKnownDirectoriesAtEachOpening = false,
                     bool addDisplayByOption = false, bool dontFilterSaveFilePathsEnteredByTheUser = false);
    ...
    IMGUI_API const char* chooseFileDialog(bool dialogTriggerButton, const char* directory = NULL,
                                           const char* fileFilterExtensionString = NULL, const char* windowTitle = NULL,
                                           const ImVec2& windowSize = ImVec2(-1, -1),
                                           const ImVec2& windowPos = ImVec2(-1, -1), const float windowAlpha = 0.875f);

    IMGUI_API const char*
    chooseFolderDialog(bool dialogTriggerButton, const char* directory = NULL, const char* windowTitle = NULL,
                       const ImVec2& windowSize = ImVec2(-1, -1), const ImVec2& windowPos = ImVec2(-1, -1),
                       const float windowAlpha = 0.875f);

    IMGUI_API const char*
    saveFileDialog(bool dialogTriggerButton, const char* directory = NULL, const char* startingFileNameEntry = NULL,
                   const char* fileFilterExtensionString = NULL, const char* windowTitle = NULL,
                   const ImVec2& windowSize = ImVec2(-1, -1), const ImVec2& windowPos = ImVec2(-1, -1),
                   const float windowAlpha = 0.875f);
    ...
    typedef bool (* DrawFileIconDelegate)(int fileExtensionType,
                                          const ImVec4* pOptionalColorOverride); // must return "true" if the icon is set.
    static DrawFileIconDelegate DrawFileIconCallback;

    typedef bool (* DrawFolderIconDelegate)(bool useOpenFolderIconIfAvailable,
                                            const ImVec4* pOptionalColorOverride);// must return "true" if the icon is set.
...
private:
    ...
    friend const char* ChooseFileMainMethod(Dialog& ist, const char* directory, const bool _isFolderChooserDialog,
...
extern void DirectoryGetDirectories(const char* directoryName, PathStringVector& result,
                                    FilenameStringVector* pOptionalNamesOut = NULL,
                                    Sorting sorting = SORT_ORDER_ALPHABETIC);

extern void
DirectoryGetFiles(const char* directoryName, PathStringVector& result, FilenameStringVector* pOptionalNamesOut = NULL,
                  Sorting sorting = SORT_ORDER_ALPHABETIC);

extern bool FileExists(const char* path);

extern bool FileGetContent(const char* path, ImVector<unsigned char>& bufferOut, bool openInTextMode = false,
                           const char* password = NULL);  // password is used if it's a file inside a zip path when IMGUI_USE_MINIZIP is defined (e.g. path="C://MyDocuments/myzipfile.zip/myzipFile/something.txt")
extern bool FileGetContent(const char* path, ImVector<char>& bufferOut, bool openInTextMode = false,
                           const char* password = NULL);  // password is used if it's a file inside a zip path when IMGUI_USE_MINIZIP is defined (e.g. path="C://MyDocuments/myzipfile.zip/myzipFile/something.txt")
extern int FileGetExtensionType(
        const char* path);  // returns one of the FileExtensionType enums, or -1. Slow: you'd better cache the result wherever possible.
extern void FileGetExtensionTypesFromFilenames(ImVector<int>& fileExtensionTypesOut,
                                               const FilenameStringVector& fileNames); // Same as above, except that now FilenameStringVector are chars of MAX_FILENAME_BYTES, usually shorter than MAX_PATH_BYTES
