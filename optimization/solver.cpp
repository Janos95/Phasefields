...
struct IpoptWrapper : Ipopt::TNLP
{
    ...
    void eval(double const* const x, double const* objectiveFactor = nullptr, double const* lambda = nullptr){
        problem.evaluate(x, &objective, gradient.data(), constraints.data(), &jacobian, &hessian, objectiveFactor, lambda);
    }
    ...
    ) override
    {
        ...
        problem.determineSparsityStructures(&jacobian, &hessian);

        nnz_jac_g = jacobian.nnz;
        nnz_h_lag = hessian.nnz;
    ...
    ) override
    {
        ...
        if(values){
            if(new_x) eval(x, &obj_factor, lambda);
            Utility::copy(hessian.values, {values, (std::size_t)nele_hess});
        } else {
            Utility::copy(hessian.rows, {iRow, (std::size_t)nele_hess});
            Utility::copy(hessian.cols, {jCol, (std::size_t)nele_hess});
        }
        return true;
...
struct FirstOrderWrapper : ceres::FirstOrderFunction {
    ...
    explicit FirstOrderWrapper(solver::Problem const& pb) : problem(pb) {
        ...
        problem.determineSparsityStructures(&jacobian, nullptr);
    }


    bool Evaluate(double const* parameters, double* cost, double* g) const override {
        SparseMatrix * ptrToJacobian = g ? &jacobian : nullptr;
        problem.evaluate(parameters, cost, nullptr, g, ptrToJacobian, nullptr, nullptr);

        if(g){
            for(Mg::UnsignedInt i = 0; i < jacobian.nnz; ++i){
